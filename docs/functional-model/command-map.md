...
| Android Device Copilot skill run | `AgentCopilot.tsx` | `adb_workbench_execute` | Runs embedded skill evidence steps against the selected online device, using the Workbench parser, risk classifier, timeout, and selected-device targeting. |
| Android Device Copilot Agent analysis | `AgentCopilot.tsx` | `agent_cli_analyze` | Runs the selected Agent CLI non-interactively after evidence collection; Codex uses read-only `codex exec`, Claude uses print mode, custom CLIs receive the prompt on stdin, and failures fall back to built-in stdout/stderr analysis. |
