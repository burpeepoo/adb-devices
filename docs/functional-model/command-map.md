...
| Scout Agent conversation | `App.tsx`, `AgentCopilot.tsx`, `src/scoutTask/` | `agent_cli_analyze`, `agent_cli_probe` | Opens from the Agent Tasks tab. The Scout workspace defaults to Feature Walkthrough, exposes Chat and Bug Repro as peer task choices, and lets either record mode submit a current-record prompt back into the same conversation stream. The CLI/model readiness chip includes a manual runtime health-check action that opens an independent modal, checks configured CLI commands with `--version`, and reports enabled API provider configuration. Each mode can optionally choose a working directory from the bottom input/action bar; that path is included in the Agent prompt and used as the CLI cwd, while blank mode state falls back to the selected CLI profile cwd. Each turn sends the user prompt, recent conversation, selected-device context, default device/performance context, current optional evidence shortcut hint, available tool contract, and returned tool results to the selected Agent CLI. Chat reads the active Scout task record when one exists; it does not create a hidden evidence record. Codex uses `codex exec` with read-only sandboxing by default unless the profile args provide `--yolo`, danger bypass, or an explicit sandbox override; Claude uses print mode, and custom CLIs receive the prompt on stdin. |
...
| Scout typed tools | `AgentCopilot.tsx`, `src/scoutTask/` | `adb_device_summary`, `adb_workbench_execute`, `adb_screenshot`, `adb_read_logcat`, `adb_list_packages`, `adb_performance_sample`, `adb_performance_stream_snapshot`, `adb_agent_status`, `adb_agent_sample` | Executes auto-approved read-only tool requests returned by the Agent as structured `toolCalls`, including the local-only `evidence.get_active_record` tool, records results into the conversation, and returns those results to the Agent for the next response. During an active `auto_execute` Scout task, the domain policy can also auto-run low- and medium-risk Workbench command requests; high-risk and Always-confirm commands still require approval. Performance context merges active stream data, ADB one-shot sampling, and Agent APK sampling when available. |
| Scout approval card | `AgentCopilot.tsx`, `src/scoutTask/` | `adb_workbench_execute` | Renders Agent-requested raw ADB commands as approval cards when policy requires explicit user action. The command is only executed through Workbench after the user chooses allow once; denial is recorded as a command result. |
| Scout evidence record | `AgentCopilot.tsx`, `src/scoutTask/` | `adb_screenshot`, `read_image_preview_data_url`, `adb_start_recording`, `adb_stop_recording`, `adb_read_logcat`, `remote_control_status`, `export_evidence_package`, `agent_cli_probe`, `agent_cli_analyze` | Captures walkthrough and Bug repro artifacts into persisted `evidenceSessions`, including screenshots, recordings, issue-time Logcat, Remote audit snapshots, notes, screen-state snapshots, Agent notes, and Markdown report export. `src/scoutTask/` owns start gates, device-bound artifact append, derived run state, final report close, and report-failure retry behavior while keeping the existing `active | closed` storage field compatible. Before creating a new Walkthrough or Bug Repro record, the UI probes the selected Agent CLI and opens runtime health instead of creating a doomed task when the command is missing. Screenshot previews use validated data URLs. Export writes a `.zip` package containing `report.md` plus available local artifact files under `assets/`. Full controls and timelines live in Walkthrough or Bug Repro mode; each mode has its own recent-record list that stretches to the available panel height, while normal Chat keeps the message stream conversation-only and does not render task-status chrome. The record footer exposes the goal field, an Auto-execute checkbox beside Start, evidence actions while running, and a single Stop and report action; the older separate header End action is not shown. Checklist or test-plan material is uploaded as conversation attachments instead of starting a separate evidence mode. |
