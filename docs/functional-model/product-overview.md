...
- Scout exposes device-bound task records for feature walkthroughs and Bug repros; these records capture notes, screenshots, Remote audit snapshots, recordings, issue-time Logcat, screen-state snapshots, Agent notes, and Markdown reports, then show the captured artifacts in the selected Walkthrough or Bug Repro timeline and recent-record list. Chat, Walkthrough, and Bug Repro each have their own history surface in the Agent Tasks left console: Chat lists recent conversations, Walkthrough lists walkthrough records, and Bug Repro lists repro records. Selecting a Walkthrough or Bug Repro history item shows that task timeline in the main evidence panel unless an active task for the mode is running. Export creates a `.zip` evidence package with `report.md` and available local artifact files under `assets/`, so screenshots travel with the report. When a task record is active, Chat mode stays focused on the conversation and does not render task-status chrome inside the message stream; users return to Walkthrough or Bug Repro from task choices or recent records to view the timeline and controls. Walkthrough and Bug Repro share a compact bottom task bar: before start, the goal field, Start button, and Auto-execute checkbox sit together; after start, Capture actions and Wrap up actions are visually separated so screenshot/record/issue actions do not compete with note/export/Stop and report. Stop closes the record only after the final report is generated; report failure keeps the task active and retryable. Checklist or test-plan material is supplied as conversation attachments for the Agent to read, not as a separate evidence mode.
