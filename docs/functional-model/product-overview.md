...
- The Agent tab is an experimental Android Device Copilot workspace. It keeps session history, routes normal multi-turn prompts to the selected Agent CLI profile, lets the agent request typed read-only device tools when needed, and keeps embedded Android-agent skills available as optional evidence shortcuts instead of forcing every prompt through a fixed workflow. A persistent bottom-right Copilot icon opens the same Copilot surface as a right-side contextual drawer on any workspace tab; the Copilot runtime health check is a manual action next to the CLI/model selector and opens an independent modal for local CLI and enabled API provider configuration. The drawer defaults to Chat mode, exposes Walkthrough and Bug Repro as peer top-level modes, checks the selected device's Agent APK status, and prompts the user to install or update the APK when it is missing.
- Copilot exposes QA Scribe evidence records for feature walkthroughs and Bug repros; these records capture notes, screenshots, Remote audit snapshots, recordings, issue-time Logcat, screen-state snapshots, Agent notes, and Markdown reports, then show the captured artifacts in the selected Walkthrough or Bug Repro timeline and recent-record list. Export creates a `.zip` evidence package with `report.md` and available local artifact files under `assets/`, so screenshots travel with the report. When a QA Scribe record is active, Chat mode stays focused on conversation and shows only a lightweight "recording" strip that links back into that record's mode. Walkthrough and Bug Repro each have a bottom blue start / red stop Agent control, where start sends the goal and current evidence to the Agent and stop generates the final QA report. Checklist or test-plan material is supplied as conversation attachments for the Agent to read, not as a separate evidence mode.
