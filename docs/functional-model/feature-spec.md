...
- The main Agent Tasks panel does not repeat the Chat/Walkthrough/Bug Repro segmented switch and does not render full-width Agent APK, accessibility, or CLI status cards. Chat, Walkthrough, and Bug Repro each expose an optional working-directory row at the bottom of the input/action area, similar to Codex: choosing a folder scopes future Agent CLI turns for that mode, clearing it returns to the selected CLI profile cwd, and leaving it blank never blocks sending or starting a task. Walkthrough and Bug Repro goals are entered near the bottom start controls so the goal sits with the action that starts the Agent task. The primary start labels stay short (`开始走查` / `开始复现`) and the Auto-execute checkbox sits in the same responsive action group beside Start, with a short tooltip that explains low/medium-risk automation and high-risk approval. The start area can wrap instead of shrinking or clipping button text. The app supports automation-friendly tab entry through hashes such as `#agent` and the development-only `VITE_ADB_MANAGER_INITIAL_TAB=agent` environment variable, while default launch still opens the Device Console. Sessions are persisted under `agentCopilotSessions`.
...
- Mutating or expert ADB commands use the `workbench.request_adb_command` path with conservative risk classification. In normal and semi-auto tasks, requests render as conversation approval cards with command, reason, copy command, deny, and allow-once actions. In `auto_execute` tasks, low- and medium-risk workbench requests can execute immediately and return their command result to the Agent; high-risk requests still require explicit user approval.
- Evidence records are user-facing, local, device-bound task records persisted under `evidenceSessions`. Starting a record captures a goal, proactive intensity (`key_moments` by default), optional per-task working directory, and execution permission (`semi_auto` by default, or `auto_execute` when the start-bar checkbox is selected), records an initial screen-state snapshot, switches the Scout surface into the matching Walkthrough or Bug Repro mode, and then tracks screenshots, notes, issue markers, recordings, Logcat, screen-state snapshots, Agent notes, and Markdown reports. Active records render a visible timeline in their selected mode; screenshot previews load through `read_image_preview_data_url` so local images are validated and returned as data URLs instead of relying on broad asset access. Export uses `export_evidence_package` to write a `.zip` with `report.md` plus available local artifact files under `assets/`; missing files are skipped and reported instead of failing the whole export. Chat mode keeps the message stream conversation-only: it may send active task context to the Agent, but task status, timeline, and controls remain in Walkthrough or Bug Repro mode surfaces. Walkthrough and Bug Repro each provide a mode-scoped recent-record list named as task records, not chats; the left task console lists only that mode's records, and selecting a record opens its full timeline in the main evidence panel. They also provide a compact Agent footer: before start, the goal field, Auto-execute checkbox, Start action, and optional directory row live together; after start, the footer separates Capture actions such as screenshot, recording, and issue marker from Wrap up actions such as note, export, and Stop and report, while keeping the task directory editable. Start first probes the selected Agent CLI command; if the command is missing or cannot run, Scout opens the runtime health panel and does not create a task record. When the probe succeeds, Start creates the evidence record and immediately sends the QA or repro goal, permission level, optional working directory, and current evidence timeline to the Agent, while Stop and report records a stop prompt, generates the final QA report, and closes the record only after report generation succeeds. If report generation fails, Scout keeps the record active, shows the failed run state, records the error, and lets the user retry. The older separate header `End` action is not shown. The Agent CLI is not kept open while the user performs a long walkthrough or repro; each CLI turn remains bounded by the backend timeout, and ADB Manager persists evidence locally until the user stops. The Agent receives the compact timeline in every turn and may call `evidence.get_active_record` for fuller detail. The Agent can also request `evidence.start_session`, but it renders as an approval card and starts only after the user allows it.
...
- `auto_execute` depends on Scout control readiness for UI-level actions and never bypasses high-risk confirmations. In the current implementation, it auto-runs low- and medium-risk workbench command requests during the active task; if Agent APK accessibility service is enabled, future UI automation can use control-level actions, otherwise automation remains limited to ADB-backed commands and screenshot/coordinate-level fallbacks.
