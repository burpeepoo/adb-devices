...
- Each control shows the supplier-facing parameter name, current value when discoverable, desired value, and readback value after apply. Editable controls also expose their input and apply action. Per-control explanatory subtitles and the top aggregate metric cards are intentionally hidden in the UI so operators can tune values without reading implementation notes or summary counters.
- Color Temperature value maps to `settings system aw_color_temperature_value` and remains visible for firmware handoff, but its card is read-only and has no direct input or apply action. The color wheel coordinate maps to `settings system srgb_color_temperature` and is edited in one full-width card containing both the clickable wheel and precise X/Y fields. Both editors share the same draft, current/target/readback group, status, and apply path, mirror Settings' 205-point coordinate space, and export as `x,y`. Applying either coordinate editor derives the native Settings ARGB color, writes it back to `aw_color_temperature_value`, and live-applies it through the display helper as `enhanceComponent[10]` / sRGB white point. The UI presents color temperature and color-wheel values as `#RRGGBB` plus `x,y` coordinates, with the signed Settings integer demoted to a "firmware raw value" detail so firmware-facing data is preserved without making the operator workflow look numeric-first. Android `settings get` returns the literal `null` for an unset key, and the UI treats that as unset rather than as a valid readback value.
...
3. Let the user edit desired values in ADB Manager, except for the read-only Color Temperature value derived from the coordinate control; if live apply is enabled, send an editable value immediately, otherwise send it when the user clicks apply.
...
- When Agent sampling is active, the frontend merges only trusted Agent health, foreground, and future explicitly sourced fields with the latest ADB persistent-stream sample and marks the source as `Agent + ADB`. ADB remains authoritative for the target PID, process RSS/PSS/threads, device network counters, system metrics, GPU, thermal, battery, storage, and `gfxinfo`. Current and older Agent self-process/UID counters are discarded rather than mislabeled as target-app metrics.
...
- Slow PSS cache entries carry the sampled package and PID and are accepted only when both still match the current target; the frontend also refuses to carry PSS across a same-package PID restart.
...
- Device headroom requires the kernel's `MemAvailable`; `MemFree` is not silently relabeled as available memory. RSS normally uses `VmRSS`, and its `/proc/<pid>/stat` fallback multiplies resident pages by the device-reported page size rather than assuming 4 KB pages.
...
- App/process: package, PID, process state, raw process CPU jiffies, PSS as the primary app-memory footprint, RSS as an auxiliary resident-set signal, and thread count.
- Device/system: raw system CPU jiffies, `MemAvailable` headroom and total memory, derived used memory for raw export compatibility, battery, thermal state, CPU frequency, GPU utilization/frequency when exposed by Android sysfs, `dumpsys gpu` memory fallback when sysfs counters are permission-limited, network bytes, `/data` storage, display size/density/refresh.
- Agent: protocol/version/permission status, device timestamp, requested target-package context, and foreground package when Usage Stats access is available. Ordinary Agent APK permissions do not make target-process memory, threads, or target-UID network counters available.
...
- Metric cards show the upper limit and current percentage only when the sample exposes a real max/total value: CPU/GPU frequency against max frequency, available memory against total memory, storage against total capacity, and GPU process memory against total GPU memory when both values are available. Device memory is labeled as available headroom rather than presenting `total - available` as memory pressure. Metrics that are already percentages, such as process CPU, system CPU, GPU usage, and jank, remain plain percentages without a synthetic `100%` limit.
...
- Draws lightweight SVG trend charts for CPU, GPU, fast-sampled app RSS, available-memory headroom, P95 frame time, and network metrics from the same rolling samples. PSS remains the primary app-memory snapshot, while RSS is retained for higher-cadence change tracking.
- Warns on missing target process, elevated thermal state, battery temperature at or above 45 C, and RSS growth over 20% when the same package/PID has a complete 5-minute baseline; jank remains visible as a metric/trend but does not raise a transient warning banner.
