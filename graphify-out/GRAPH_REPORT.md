# Graph Report - adb_project  (2026-05-15)

## Corpus Check
- 37 files · ~84,621 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 234 nodes · 394 edges · 11 communities detected
- Extraction: 79% EXTRACTED · 21% INFERRED · 0% AMBIGUOUS · INFERRED: 81 edges (avg confidence: 0.82)
...
1. `run_adb_with_timeout()` - 15 edges
2. `ensure_success()` - 15 edges
3. `run_adb()` - 14 edges
...
9. `adb_export_package_apk()` - 8 edges
10. `adb_start_logcat()` - 8 edges
...
Cohesion: 0.12
Nodes (31): run_adb_with_env_timeout(), run_adb_with_timeout(), wait_with_timeout(), adb_auto_connect(), adb_connect(), adb_devices(), adb_disconnect(), adb_mdns_auto_connect() (+23 more)
...
Cohesion: 0.15
Nodes (17): ensure_success(), send_navigation_key(), adb_export_package_apk(), adb_list_package_details(), adb_list_packages(), adb_package_info(), apk_output_file_name(), ExportedApk (+9 more)

### Community 3 - "Community 3"
Cohesion: 0.16
Nodes (8): AdbError, adb_screenshot(), download_with_progress(), emit_install_progress(), get_default_save_dir(), install_adb(), open_file(), reveal_path()
...
Cohesion: 0.27
Nodes (12): build_adb_command(), check_adb_available(), ensure_executable(), get_adb_path(), get_bundled_adb_path(), get_sdk_adb_path(), get_system_adb_path(), run_adb() (+4 more)
...
## Knowledge Gaps
- **8 isolated node(s):** `DeviceInfo`, `MdnsDevice`, `ScreenMirrorState`, `GithubRelease`, `GithubAsset` (+3 more)
...
- **Why does `run_adb()` connect `Community 6` to `Community 0`, `Community 2`, `Community 3`, `Community 4`, `Community 7`?**
  _High betweenness centrality (0.101) - this node is a cross-community bridge._
- **Why does `ensure_success()` connect `Community 2` to `Community 0`, `Community 1`, `Community 3`, `Community 6`, `Community 7`?**
  _High betweenness centrality (0.089) - this node is a cross-community bridge._
- **Why does `run_adb_with_timeout()` connect `Community 0` to `Community 1`, `Community 2`, `Community 6`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Are the 12 inferred relationships involving `run_adb_with_timeout()` (e.g. with `adb_restart_server()` and `adb_devices()`) actually correct?**
  _`run_adb_with_timeout()` has 12 INFERRED edges - model-reasoned connections that need verification._
- **Are the 14 inferred relationships involving `ensure_success()` (e.g. with `adb_mdns_discover()` and `adb_mdns_auto_connect()`) actually correct?**
  _`ensure_success()` has 14 INFERRED edges - model-reasoned connections that need verification._
- **Are the 12 inferred relationships involving `run_adb()` (e.g. with `adb_list_packages()` and `adb_package_info()`) actually correct?**
